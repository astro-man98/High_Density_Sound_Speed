...
class RVSS_EOS_Slope:
    def __init__(self, mu_i, cs2_i = 0.95, mu_max = 1500, nmatch = 1.0):
        ...
        n_mesh = 100
        mu_mesh_regI = np.linspace(chem_pot_compose[-1],mu_i,n_mesh)
        mu_mesh_regII = np.linspace(1.01*mu_i,mu_max,n_mesh)

        beta = 1/(cs2_m + alpha*mu_m)

        cs2_hd = np.empty(2*n_mesh)
        nb_hd = np.empty(2*n_mesh)
        P_hd = np.empty(2*n_mesh)
        e_hd = np.empty(2*n_mesh)
        mu_hd = np.concatenate([mu_mesh_regI,mu_mesh_regII])

        for i,mu in enumerate(mu_hd):
            if mu < mu_i:
                cs2_hd[i] = alpha*(mu - mu_m) + cs2_m
            else:
                cs2_hd[i] = cs2_i

        cs2_interp = interp1d(mu_hd,cs2_hd)
        for i,mu in enumerate(mu_hd):
            integral_nb = quad(lambda mu_p: 1/(mu_p*cs2_interp(mu_p)),mu_m,mu)
            nb_hd[i] = n0*np.exp(integral_nb[0])

        nb_interp = interp1d(mu_hd,nb_hd)

        for i,mu in enumerate(mu_hd):
            integral_pr = press_compose[-1] + quad(lambda mu_p: nb_interp(mu_p),mu_m,mu,limit=200)
            P_hd[i] = integral_pr[0]
            e_hd[i] = mu*nb_hd[i] - P_hd[i]

        self.P_unified = np.concatenate([press_compose,P_hd[1:]])
        self.E_unified = np.concatenate([eden_compose,e_hd[1:]])
